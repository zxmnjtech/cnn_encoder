...
class Parallel(nn.Module):
    def __init__(self):
        ...
        self.fc1_linear = nn.Linear(820, 4)
        self.softmax_out = nn.Softmax(dim=1)

    def forward(self, x):
        x1,x2,x3=self.cnn(x)
        y1,y2,y3=self.Transformerr(x)
...
class Cnn_Transformer(nn.Module):
    def __init__(self, num_emotions):
        ...
        self.fc1_linear = nn.Linear(780, num_emotions)
        self.softmax_out = nn.Softmax(dim=1)
...
if __name__ == '__main__':
    ...
    featuresFileName_Change = 'features_{}_{}_1.pkl'.format(FEATURES_TO_USE, impro_or_script)
    featuresExist = True
    toSaveFeatures = True
    WAV_PATH = "D:\Download\IEMOCAP/"
    # WAV_PATH = "D:/ravdess/Actor_*/"
    RATE = 16000
    path = 'models/augment_1.pth'
    ...
    if (featuresExist == True):
        with open(featuresFileName_Change, 'rb')as f:
        ...
        if (toSaveFeatures == True):
            ...
            with open(featuresFileName_Change, 'wb') as f:
