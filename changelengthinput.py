...
class Cnn_Transformer(nn.Module):
    def __init__(self, num_emotions):
        ...
        self.transformer_encoder = nn.TransformerEncoder(transformer_layer, num_layers=16)
        ...
        self.fc1_linear = nn.Linear(3112, num_emotions)  # 2344
        self.softmax_out = nn.Softmax(dim=1)

    def forward(self, x, x_next):  # 输入的形状为32*1*40*63
...
if __name__ == '__main__':
    ...
    featuresFileName = 'features_{}_{}_fold5second8aug1.pkl'.format(FEATURES_TO_USE, impro_or_script)
    featuresExist = True
    toSaveFeatures = True
    ...
    # data processing
    def process_data(path, t=8, train_overlap=1, val_overlap=1.6, RATE=16000, dataset='iemocap'):
        path = path.rstrip('/')
        ...
        # n是IEMOCAP数据集的长度
        n = len(wav_files)
        # n_aug是基于VTLP数据增强的数据集的长度和n的长度是一样的
